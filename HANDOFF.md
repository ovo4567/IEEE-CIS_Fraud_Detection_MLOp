...
| MLflow store | deployment registry: `models/seed/mlflow.db` (clean store seeded from the committed artifact; legacy `mlruns/mlflow.db` is archaeology, intentionally not reused) |
...
| Docker Compose demo | `deploy/compose.yaml` + `make demo` — 4 services on one image, offline from the committed seed (MLflow 5001, Prefect 4200, API 8000, worker); see "This session" below |
...
Docs only, no code changed (suite stays 151 passing). The portfolio-facing
documentation is now real:

- **`README.md`** — rewritten from the cookiecutter boilerplate: one-paragraph
  pitch + key numbers (test AUC 0.9286, threshold 0.0551), a Mermaid
  **architecture diagram** of the closed loop (train → serve → monitor →
  retrain → promote), a **`make demo` quickstart** (prereqs + service table +
  URLs), the one-command loop (`make simulate`/`monitor`/`retrain`), CI/CD
  summary, and a repo layout that reflects the real package (`serving`,
  `orchestration`, `monitoring`, `deployment`).
- **`docs/docs/portfolio.md`** — the MLOps story for reviewers: the problem,
  the model, the ADR decisions, the three-phase closed loop, why it's a
  credible deployment (fresh-clone verification, honest label arrival, real
  DeLong gate, CI gating), what to run, and honest limitations/v2 ideas.
- **MkDocs** — `docs/mkdocs.yml` gains a nav (Home / Getting started /
  Portfolio); `docs/docs/index.md` is now a landing page linking the
  portfolio, getting-started, ADRs, and CONTEXT.md.
